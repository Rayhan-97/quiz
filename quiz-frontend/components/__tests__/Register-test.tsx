...
import {
  ...
  GetByQuery,
  QueryAllByQuery,
} from "@testing-library/react-native/build/queries/makeQueries";
...
describe("registration component test", () => {
  ...
  let findByTestId: FindByQuery<
    TextMatch,
    CommonQueryOptions & TextMatchOptions
  >;
  let queryAllByText: QueryAllByQuery<
    TextMatch,
    CommonQueryOptions & TextMatchOptions
  >;
  ...
  it("given invalid response when submit form then display error message", async () => {
    ...
    expect(submissionError.children[0]).toMatch(
      "Error code <35>. Something has gone terribly wrong",
    );
